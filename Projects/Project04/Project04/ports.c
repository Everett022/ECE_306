...
void Init_Port_6(void){
    ...
    P6SEL0 &= ~R_FORWARD;       //Set select for GP I/O to 0
    P6SEL1 &= ~R_FORWARD;       //Set select for GP I/O to 0
    P6OUT &= ~R_FORWARD;        //Set the port pin to off
    P6DIR |= R_FORWARD;         //Set the direction to output

    P6SEL0 &= ~L_FORWARD;       //Set select for GP I/O to 0
    P6SEL1 &= ~L_FORWARD;       //Set select for GP I/O to 0
    P6OUT &= ~L_FORWARD;        //Set the port pin to off
    P6DIR |= L_FORWARD;         //Set the direction to output

    P6SEL0 &= ~R_REVERSE;       //Set select for GP I/O to 0
    P6SEL1 &= ~R_REVERSE;       //Set select for GP I/O to 0
    P6OUT &= ~R_REVERSE;        //Set the port pin to off
    P6DIR |= R_REVERSE;         //Set the direction to output

    P6SEL0 &= ~L_REVERSE;       //Set select for GP I/O to 0
    P6SEL1 &= ~L_REVERSE;       //Set select for GP I/O to 0
